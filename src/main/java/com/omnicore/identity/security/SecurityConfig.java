  ...
  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    return http.csrf(csrf -> csrf.disable())
        ...
        .authorizeHttpRequests(
            auth ->
                auth.requestMatchers(HttpMethod.POST, ApiPaths.AUTH_BASE_PATH + ApiPaths.LOGIN_PATH)
                    .permitAll()
                    .requestMatchers(ApiPaths.API_V1 + "/**")
                    .authenticated()
                    .requestMatchers(ApiPaths.LEGACY_API_PREFIX + "/**")
                    .denyAll()
                    .anyRequest()
                    .permitAll())
        .oauth2ResourceServer(
            oauth2 ->
                oauth2.jwt(
                    jwt -> jwt.jwtAuthenticationConverter(jwtPermissionAuthenticationConverter)))
        .build();
  }
